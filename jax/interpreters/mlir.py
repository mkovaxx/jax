...
def lower_jaxpr_to_fun(ctx: LoweringContext, name: str,
  ...
  if prune_tokens:
    pruned_in_avals = [aval for aval in jaxpr.in_avals
                       if not isinstance(aval, core.AbstractToken)]
  ...
  with ir.InsertionPoint(entry_block):
    ...
    if prune_tokens and len(pruned_in_avals) != len(jaxpr.in_avals):
      token = mhlo.CreateTokenOp(aval_to_ir_type(core.AbstractToken())).results
      arg_iter = iter(unflattened_args)
      unflattened_args = [
          token if isinstance(aval, core.AbstractToken) else next(arg_iter)
