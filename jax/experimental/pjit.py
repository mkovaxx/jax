...
def pjit(fun: Callable,
  ...
  def infer_params(*args, **kwargs):
    ...
    in_positional_semantics = tuple(
        ...
        if isinstance(a, GSDA) else maps._positional_semantics
        for a in args_flat)
...
def _pjit_lower(
  ...
  f.__name__ = name
  fun = lu.wrap_init(f)
  local_in_avals = global_to_local(in_positional_semantics, resource_env.physical_mesh,
                                   jaxpr.in_avals, in_axis_resources)
  return pxla.lower_mesh_computation(
      fun, name, resource_env.physical_mesh,
      in_axes, out_axes, donated_invars,
      True, local_in_avals, False, in_positional_semantics)
...
def _pjit_partial_eval(trace, *in_tracers,
  ...
  unknown_params = dict(
      ...
      donated_invars=(keep_where(donated_invars, unknown_ins) +
                      (False,) * num_residuals),
      name=name,
      # FIX THIS! DO_NOT_SUBMIT. False is wrong there. Should I replace with
      # maps._positional_semantics?
      in_positional_semantics=(keep_where(
          in_positional_semantics, unknown_ins) + (False,) * num_residuals),
      out_positional_semantics=out_positional_semantics)
...
def _pjit_transpose(reduce_axes, cts_in, *primals_in,
  ...
  # DO_NOT_SUBMIT FIX THIS! What to do about out_axis_resources that's being
  # replaced with in_positional_semantics
  transpose_in_positional_semantics = (
    *prune_type(ad.UndefinedPrimal, in_positional_semantics, primals_in),
    *prune_type(ad.Zero, in_positional_semantics, cts_in)
  )
